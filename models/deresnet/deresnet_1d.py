...
def deconv3x3(in_planes, out_planes, stride=(1, 1), output_padding=(0, 0)):
    return nn.ConvTranspose2d(in_planes, out_planes, kernel_size=(1, 3), stride=stride,
                              padding=(0, 1), output_padding=output_padding, bias=False)
...
class DeconvResNet(nn.Module):
    def __init__(self, block, layers):
        ...
        self.layer_start = nn.ConvTranspose2d(256, 128, kernel_size=(1, 7), stride=(1, 2), padding=(0, 1),
                                              output_padding=(0, 0), bias=False)
        self.layer2 = self._make_layer(block, 128, layers[2], stride=(1, 2))
        self.layer1 = self._make_layer(block, 64, layers[1], stride=(1, 2), output_padding=(0, 1))
        self.layer0 = self._make_layer(block, 64, layers[0])

        self.conv1 = nn.ConvTranspose2d(64, 64, kernel_size=(1, 3), stride=(1, 2), padding=(0, 1),
                                        output_padding=(0, 1), bias=False)
        self.bn1 = nn.BatchNorm2d(64)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = nn.ConvTranspose2d(64, 1, kernel_size=(1, 7), stride=(1, 2), padding=(0, 3),
                                        output_padding=(0, 1), bias=False)

    def _make_layer(self, block, planes, blocks, stride=(1, 1), output_padding=(0, 0)):
        upsample = None
        if stride != (1, 1) or self.inplanes != planes * block.expansion:
            upsample = nn.Sequential(
                nn.ConvTranspose2d(self.inplanes, planes * block.expansion,
                                   kernel_size=(1, 1), stride=stride, bias=False, output_padding=output_padding),
