...
The percentage of co-methylation (PCM) is calculated as the ratio of the co-methylated reads and the overall reads within a region, where a read is co-methylated if there were a methylated sites within b consecutive CpG sites. a and b are the parameters for this metric. We implement two popular settings in this paper, i.e., (a = 2, b = 3) and (a = 3, b = 5).
...
We grid search the candidate hyper-parameters listed in Table~\ref{tab:classifier_configuration}. The best hyper-parameters for each experiment are shown in Table~\ref{tab:best_hparams}.
